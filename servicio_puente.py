...
def consultar_vehiculo_robado(conn_mobileapps, conn_robados, placa, evento):
    try:
        ...
        if cursor_robados.fetchone():
            ...
            # Aquí enviamos el JSON a la URL especificada
            enviar_alerta_vehiculo_robado(evento)
...
def obtener_eventos():
    """Obtiene eventos del endpoint y los procesa."""
    conn_mobileapps = conectar_a_sql(server_mobileapps, database_mobileapps)
    conn_robados = conectar_a_sql(server_robados, database_robados)

    if not conn_mobileapps or not conn_robados:
        print("No se pudo establecer la conexión con las bases de datos.")
        return

    try:
        # Configurar solicitud GET con timeout y streaming habilitado
        response = requests.get(url, headers=headers, timeout=10, stream=True)
        response.raise_for_status()  # Verifica si hubo errores HTTP

        print("Conectado al servidor. Procesando eventos...\n")

        # Procesar eventos como flujo continuo
        for line in response.iter_lines():
            if line:
                line_decoded = line.decode("utf-8")
                if line_decoded.startswith("data:"):
                    # Quitar la parte 'data:' del principio y tratar de decodificar el resto
                    try:
                        evento_json = json.loads(line_decoded[5:])  # Descartar "data:" y decodificar
                        print("Evento recibido:", json.dumps(evento_json, indent=4))

                        # Insertar el evento en la base de datos MobileApps
                        insertar_evento(conn_mobileapps, conn_robados, evento_json)

                    except json.JSONDecodeError:
                        error_logger.error(f"Error al decodificar el evento: {line_decoded}")
                else:
                    error_logger.error(f"Mensaje no válido para decodificación como evento: {line_decoded}")

    except requests.exceptions.Timeout:
        error_logger.error("Error: La solicitud excedió el tiempo de espera.")
    except requests.exceptions.RequestException as e:
        error_logger.error(f"Error al realizar la solicitud: {e}")
    except KeyboardInterrupt:
        print("\nEjecución interrumpida por el usuario.")
    finally:
        conn_mobileapps.close()
        conn_robados.close()
